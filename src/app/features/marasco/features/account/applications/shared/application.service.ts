...
export class ApplicationService {
  ...
  all(): Observable<any> {
    return this._authHttp.get(this._url).pipe(
      map((applications: any) => applications),
  ...
  getDetails(id: string): Observable<[IApplication]> {
  ...
  get(id: string): Observable<IApplication> {
  ...
  insert(application: IApplication): Observable<IApplication> {
    return this._authHttp.post(this._url, JSON.stringify(application)).pipe(
      map((client: any) => client),
      catchError(this.handleError)
    );
  }
  ...
  update(application: IApplication): Observable<IApplication> {
    return this._authHttp
      ...
      .pipe(
        map((application: any) => application),
        catchError(this.handleError)
      );
  }
